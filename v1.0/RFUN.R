...
simulateHMM <- function(model, sel_cof, dom_par, pop_siz, int_con, smp_lab, smp_gen, smp_qua, thr_val, ...) {
  ...
  while (TRUE) {
    ...
    if (model == "WFD") {
      mut_frq <- cmpsimulateWFD(sel_cof, dom_par, pop_siz, int_con, int_gen, lst_gen, ptn_num, dat_aug = FALSE)
...
#' Standard version
runBPF <- function(sel_cof, dom_par, pop_siz, raw_smp, ptn_num, pcl_num) {
  ...
  # run the BPF
  BPF <- runBPF_arma(sel_cof, dom_par, pop_siz, raw_smp, ptn_num, pcl_num)
...
#' Standard version
calculateOptimalParticleNum <- function(sel_cof, dom_par, pop_siz, raw_smp, ptn_num, pcl_num, gap_num) {
  # preprocess the raw sample
  raw_smp <- as.data.frame(raw_smp)
  rownames(raw_smp) <- NULL
  colnames(raw_smp) <- c("id", "generation", "A0A0", "A0A1", "A1A1")
  ...
  OptNum <- calculateOptimalParticleNum_arma(sel_cof, dom_par, pop_siz, raw_smp, ptn_num, pcl_num, gap_num)
...
#' Standard version
runPMMH <- function(sel_cof, dom_par, pop_siz, raw_smp, ptn_num, pcl_num, itn_num) {
  # preprocess the raw sample
  raw_smp <- as.data.frame(raw_smp)
  rownames(raw_smp) <- NULL
  colnames(raw_smp) <- c("id", "generation", "A0A0", "A0A1", "A1A1")
  ...
  # run the PMMH
  PMMH <- runPMMH_arma(sel_cof, dom_par, pop_siz, raw_smp, ptn_num, pcl_num, itn_num)
...
#' Standard version
runBayesianProcedure <- function(sel_cof, dom_par, pop_siz, raw_smp, ptn_num, pcl_num, itn_num, brn_num, thn_num) {
  # preprocess the raw sample
  raw_smp <- as.data.frame(raw_smp)
  rownames(raw_smp) <- NULL
  colnames(raw_smp) <- c("id", "generation", "A0A0", "A0A1", "A1A1")
