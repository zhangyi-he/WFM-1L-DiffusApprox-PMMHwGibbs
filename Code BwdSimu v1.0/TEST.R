...
# call R functions
source("./Code/Code v1.0/Code BkdSimu v1.0/RFUN.R")
...
#' Simulate the mutant allele frequency trajectory according to the single-locus Wright-Fisher backward diffusion with selection
#' Parameter setting
#' @param sel_cof the selection coefficient
#' @param dom_par the dominance parameter
#' @param pop_siz the size of the horse population (non-constant)
...
lst_gen <- 500
ptn_num <- 5e+00
n_x <- round(2 * max(pop_siz) * 0.05) + 1
n_t <- round(n_x * n_x / 8 / min(pop_siz)) + 1
ptn_num <- c(n_t, n_x - 20, 27)
unif_grd <- TRUE
coord_x <- calculateGrid_arma(pop_siz, ref_siz, ptn_num, unif_grd)
lst_frq <- coord_x[10]

plot(1:(length(coord_x)-1), diff(coord_x), type = 'l', lwd = 1.5,
     xlab = "x", ylab = "Delta x")
abline(h = 1/n_x, col = 'red')

frq_pth <- cmpsimulateBackWFD(sel_cof, dom_par, pop_siz, ref_siz, lst_frq, evt_gen, int_gen, lst_gen, ptn_num, unif_grd, dat_aug = TRUE)$mut_frq_pth

t <- (int_gen:(int_gen + (lst_gen - int_gen) * ptn_num[1])) / 2 / ref_siz
plot(t, frq_pth, type = 'l', lwd = 1.5,
     xlab = "Generation", ylab = "Allele frequency",
     main = "WFD: the mutant allele frequency trajectory")
...
#' Compare the simulation generated with the KBE and the backward Wright-Fisher diffusion
sel_cof <- c(1e-03, 2e-03)
dom_par <- 5e-01
pop_siz <- c(rep(1e+04, length.out = 201), rep(5e+03, length.out = 200), rep(1e+04, length.out = 100))
ref_siz <- 1e+04
evt_gen <- 250
...
coord_x <- calculateGrid_arma(pop_siz, ref_siz, ptn_num, unif_grd)
lst_frq <- coord_x[100]
lst_frq_dist <- rep(0, length(coord_x))
lst_frq_dist[100] <- 1
sim_num <- 1e+05
dat_aug = TRUE

smp_KBE <- numeric(sim_num)
smp_backWFD <- numeric(sim_num)
log_w_backWFD <- numeric(sim_num)
p_KBE <- cmpcalculateTransitionProb_Back(sel_cof, dom_par, pop_siz, ref_siz, lst_frq_dist, evt_gen, int_gen, lst_gen, ptn_num, dat_aug)
smp_KBE <- sample(coord_x, size=sim_num, replace = TRUE, p_KBE$p_tran[,1])

for (i in 1:sim_num) {
  print(i)
  backWFD <- cmpsimulateBackWFD(sel_cof, dom_par, pop_siz, ref_siz, lst_frq, evt_gen, int_gen, lst_gen, ptn_num, dat_aug)
  smp_backWFD[i] <- backWFD$mut_frq_pth[1]
  path_wght = backWFD$wght
  log_w_backWFD[i] <- sum(log(path_wght))
}
resmp_backWFD <- sample(smp_backWFD, size=sim_num, replace = TRUE, exp(log_w_backWFD))
hist(smp_backWFD, breaks = seq(min(smp_backWFD, smp_KBE, resmp_backWFD), max(smp_backWFD, smp_KBE, resmp_backWFD), length.out = 50), freq = FALSE, col = rgb(0.1, 0.1, 0.1, 0.5),
     xlim = c(min(smp_backWFD, smp_KBE, resmp_backWFD), max(smp_backWFD, smp_KBE, resmp_backWFD)),
     xlab = "Allele frequency", main = paste("Histograms of the mutant allele frequency at generation", lst_gen))
hist(smp_KBE, breaks = seq(min(smp_backWFD, smp_KBE, resmp_backWFD), max(smp_backWFD, smp_KBE, resmp_backWFD), length.out = 50), freq = FALSE, col = 'red', add = TRUE)
hist(resmp_backWFD, breaks = seq(min(smp_backWFD, smp_KBE, resmp_backWFD), max(smp_backWFD, smp_KBE, resmp_backWFD), length.out = 50), freq = FALSE, col = rgb(0.8, 0.8, 0.8, 0.5), add = TRUE)

hist(exp(log_w_backWFD), freq = FALSE, main = paste("Histograms of the weight w of backward diffusion", lst_gen))
################################################################################
...
smp_gen <- tru_smp[, 1]
smp_siz <- rowSums(tru_smp[, -1])
smp_cnt <- t(as.matrix(tru_smp[, 2:4]))
...
save(model, sel_cof, dom_par, pop_siz, int_con, evt_gen, smp_lab, smp_gen, smp_qua, thr_val, sim_HMM_WFM,
     file = "./Output/Output v1.0/TEST v1.4/TEST_SimData.rda")

load("./Output/Output v1.0/TEST v1.4/TEST_SimData.rda")
...
pdf(file = "./Output/Output v1.0/TEST v1.4/TEST_SimData.pdf", width = 16, height = 12)
...
save(sel_cof, dom_par, pop_siz, ref_siz, evt_gen, frq_pth, raw_smp, ptn_num, pcl_num, BPF,
     file = "./Output/Output v1.0/TEST v1.4/TEST_BPF.rda")

load("./Output/Output v1.0/TEST v1.4/TEST_BPF.rda")
...
pdf(file = "./Output/Output v1.0/TEST v1.4/TEST_BPF_Likelihood.pdf", width = 8, height = 6)
...
pdf(file = "./Output/Output v1.0/TEST v1.4/TEST_BPF_Particle.pdf", width = 24, height = 66)
...
save(sel_cof, dom_par, pop_siz, ref_siz, evt_gen, frq_pth, raw_smp, ptn_num, pcl_num, gap_num, OptNum,
     file = "./Output/Output v1.0/TEST v1.4/TEST_OptNum.rda")

load("./Output/Output v1.0/TEST v1.4/TEST_OptNum.rda")

opt_pcl_num <- OptNum$opt_pcl_num
log_lik_sdv <- OptNum$log_lik_sdv

pdf(file = "./Output/Output v1.0/TEST v1.4/TEST_OptNum.pdf", width = 8, height = 6)
par(mar = c(5.5, 5, 5.5, 2.5), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
...
save(sel_cof, dom_par, pop_siz, ref_siz, evt_gen, raw_smp, ptn_num, pcl_num, itn_num, PMMH,
     file = "./Output/Output v1.0/TEST v1.4/TEST_PMMH.rda")

load("./Output/Output v1.0/TEST v1.4/TEST_PMMH.rda")

sel_cof_chn <- PMMH$sel_cof_chn
frq_pth_chn <- PMMH$frq_pth_chn
cal_smp_chn <- PMMH$cal_smp_chn

pdf(file = "./Output/Output v1.0/TEST v1.4/TEST_PMMH_Traceplot.pdf", width = 16, height = 6)
...
pdf(file = "./Output/Output v1.0/TEST v1.4/TEST_PMMH_Posterior_SelCoeff.pdf", width = 16, height = 6)
par(mfrow = c(1, 2), mar = c(5.5, 5, 5.5, 2.5), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
...
pdf(file = "./Output/Output v1.0/TEST v1.4/TEST_PMMH_Posterior_SelChange.pdf", width = 12, height = 6)
...
pdf(file = "./Output/Output v1.0/TEST v1.4/TEST_PMMH_Posterior_Traj.pdf", width = 12, height = 6)
par(mar = c(5.1, 5.1, 4.1, 1.1), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
...
save(sel_cof, dom_par, pop_siz, ref_siz, evt_gen, raw_smp, ptn_num, pcl_num, itn_num, stp_siz, apt_rto, PMMH,
     file = "./Output/Output v1.0/TEST v1.4/TEST_AdaptPMMH.rda")

load("./Output/Output v1.0/TEST v1.4/TEST_AdaptPMMH.rda")

sel_cof_chn <- PMMH$sel_cof_chn
frq_pth_chn <- PMMH$frq_pth_chn
cal_smp_chn <- PMMH$cal_smp_chn

pdf(file = "./Output/Output v1.0/TEST v1.4/TEST_AdaptPMMH_Traceplot.pdf", width = 16, height = 6)
par(mfrow = c(1, 2), mar = c(5.5, 5, 5.5, 2.5), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
plot(1:itn_num, sel_cof_chn[1, 1:itn_num], type = 'l',
     xlab = "Iteration", ylab = "Selection coefficient",
     main = "Trace plot for selection coefficient before event")
...
pdf(file = "./Output/Output v1.0/TEST v1.4/TEST_AdaptPMMH_Posterior_SelCoeff.pdf", width = 16, height = 6)
par(mfrow = c(1, 2), mar = c(5.5, 5, 5.5, 2.5), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
hist(sel_cof_chn[1, ], breaks = seq(min(sel_cof_chn[1, ]), max(sel_cof_chn[1, ]), length.out = 50), freq = FALSE,
     xlab = "Selection coefficient",
     main = "Posterior for selection coefficient before event")
...
pdf(file = "./Output/Output v1.0/TEST v1.4/TEST_AdaptPMMH_Posterior_SelChange.pdf", width = 12, height = 6)
par(mar = c(5.1, 5.1, 4.1, 1.1), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
hist(dif_sel_chn, breaks = seq(min(dif_sel_chn), max(dif_sel_chn), length.out = 50), freq = FALSE,
     xlab = "Change in selection coefficient",
     main = "Posterior for change in selection coefficient")
...
pdf(file = "./Output/Output v1.0/TEST v1.4/TEST_AdaptPMMH_Posterior_Traj.pdf", width = 12, height = 6)
par(mar = c(5.1, 5.1, 4.1, 1.1), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
plot(0, type = 'n', xlim = c(min(smp_gen), max(smp_gen)), ylim = c(min(frq_pth_chn, sim_HMM_WFM$mut_frq), max(frq_pth_chn, sim_HMM_WFM$mut_frq)),
     xlab = "Generation", ylab = "Allele frequency",
     main = "Posterior for underlying trajectory of mutant allele")
...
save(sel_cof, dom_par, pop_siz, ref_siz, evt_gen, raw_smp, ptn_num, pcl_num, itn_num, brn_num, thn_num, adp_set, stp_siz, apt_rto, BayesianProcedure,
     file = "./Output/Output v1.0/TEST v1.4/TEST_BayesProc.rda")

load("./Output/Output v1.0/TEST v1.4/TEST_BayesProc.rda")
...
pdf(file = "./Output/Output v1.0/TEST v1.4/TEST_BayesProc_Posterior_SelCoeff.pdf", width = 16, height = 6)
par(mfrow = c(1, 2), mar = c(5.5, 5, 5.5, 2.5), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
hist(sel_cof_chn[1, ], breaks = seq(min(sel_cof_chn[1, ]), max(sel_cof_chn[1, ]), length.out = 50), freq = FALSE,
     xlab = "Selection coefficient",
     main = "Posterior for selection coefficient before event")
...
pdf(file = "./Output/Output v1.0/TEST v1.4/TEST_BayesProc_Posterior_SelChange.pdf", width = 12, height = 6)
par(mar = c(5.1, 5.1, 4.1, 1.1), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
hist(dif_sel_chn, breaks = seq(min(dif_sel_chn), max(dif_sel_chn), length.out = 50), freq = FALSE,
     xlab = "Change in selection coefficient",
     main = "Posterior for change in selection coefficient")
...
pdf(file = "./Output/Output v1.0/TEST v1.4/TEST_BayesProc_Posterior_Traj.pdf", width = 12, height = 6)
par(mar = c(5.1, 5.1, 4.1, 1.1), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
plot(0, type = 'n', xlim = c(min(smp_gen), max(smp_gen)), ylim = c(min(frq_pth_chn, sim_HMM_WFM$mut_frq), max(frq_pth_chn, sim_HMM_WFM$mut_frq)),
     xlab = "Generation", ylab = "Allele frequency",
     main = "Posterior for underlying trajectory of mutant allele")
...
#' Run the Bayesian Procedure for the inference of the selection coefficient using backward simulation
#' Parameter settings
#' @param sel_cof the selection coefficient
#' @param dom_par the dominance parameter
#' @param pop_siz the size of the horse population (non-constant)
...
system.time(BackBayesianProcedure <- cmprunBackBayesianProcedure(sel_cof, dom_par, pop_siz, ref_siz, evt_gen, raw_smp, ptn_num, unif_grd, pcl_num, itn_num, brn_num, thn_num, adp_set, stp_siz, apt_rto))
...
sel_cof_chn <- BackBayesianProcedure$sel_cof_chn
sel_cof_est <- BackBayesianProcedure$sel_cof_est
sel_cof_hpd <- BackBayesianProcedure$sel_cof_hpd

pdf(file = "./Output/Output v1.0/TEST v1.4/TEST_BackBayesProc_Posterior_SelCoeff.pdf", width = 16, height = 6)
par(mfrow = c(1, 2), mar = c(5.5, 5, 5.5, 2.5), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
hist(sel_cof_chn[1, ], breaks = seq(min(sel_cof_chn[1, ]), max(sel_cof_chn[1, ]), length.out = 50), freq = FALSE,
     xlab = "Selection coefficient",
     main = "Posterior for selection coefficient before event")
...
dif_sel_chn <- BackBayesianProcedure$dif_sel_chn
dif_sel_est <- BackBayesianProcedure$dif_sel_est
dif_sel_hpd <- BackBayesianProcedure$dif_sel_hpd

pdf(file = "./Output/Output v1.0/TEST v1.4/TEST_BackBayesProc_Posterior_SelChange.pdf", width = 12, height = 6)
par(mar = c(5.1, 5.1, 4.1, 1.1), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
hist(dif_sel_chn, breaks = seq(min(dif_sel_chn), max(dif_sel_chn), length.out = 50), freq = FALSE,
     xlab = "Change in selection coefficient",
     main = "Posterior for change in selection coefficient")
...
frq_pth_chn <- BackBayesianProcedure$frq_pth_chn
frq_pth_est <- BackBayesianProcedure$frq_pth_est
frq_pth_hpd <- BackBayesianProcedure$frq_pth_hpd

pdf(file = "./Output/Output v1.0/TEST v1.4/TEST_BackBayesProc_Posterior_Traj.pdf", width = 12, height = 6)
par(mar = c(5.1, 5.1, 4.1, 1.1), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
plot(0, type = 'n', xlim = c(min(smp_gen), max(smp_gen)), ylim = c(min(frq_pth_chn, sim_HMM_WFM$mut_frq), max(frq_pth_chn, sim_HMM_WFM$mut_frq)),
     xlab = "Generation", ylab = "Allele frequency",
     main = "Posterior for underlying trajectory of mutant allele")
...
cal_smp_est <- BackBayesianProcedure$cal_smp_est
